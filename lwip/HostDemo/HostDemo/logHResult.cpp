...
#define CASE_FACILITY(f)  \
        case f: \
            StringCchCopyA(szFacility,sizeof(szFacility),#f); \
            break;

#define CASE_HRESULT(hr)  \
        case hr: \
            StringCchCopyA(szErrorName,sizeof(szErrorName),#hr); \
...
#define CASE_CODE(c) \
        case c: \
            StringCchCopyA(szErrorName,sizeof(szErrorName),#c); \
            FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM, NULL, hr, \
                MAKELANGID(LANG_NEUTRAL, SUBLANG_NEUTRAL), \
                szErrorDesc, sizeof(szErrorDesc), NULL);\
            break; 
...
void logHRESULT::getValue(std::string &toAppend) const
{
	FormatError(_hr,toAppend);
}

/*static*/ void logHRESULT::FormatError(HRESULT hr,std::string &toAppend)
{
    char szBuf[1024];
    char szFacility[100];
    char szErrorName[100];
    char szErrorDesc[400];
    ...
    *szErrorDesc=0;
    ...
    switch (HRESULT_FACILITY(hr))
    {
        ...
        default:
            StringCchCopyA(szFacility,sizeof(szFacility), "Unknown or new FACILITY code");
    }
    ...
    {  
		...
		// String Safe
        CASE_HRESULT(STRSAFE_E_INSUFFICIENT_BUFFER)
//      CASE_HRESULT(STRSAFE_E_INVALID_PARAMETER)
        CASE_HRESULT(STRSAFE_E_END_OF_FILE)
        ...
        default:
            ...
            if (HRESULT_FACILITY(hr) == FACILITY_WIN32)
            {
                ...
                default:
                StringCchCopyA(szErrorName,sizeof(szErrorName),"Unknown or new error");
                FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM, NULL, hr, 
                        MAKELANGID(LANG_NEUTRAL, SUBLANG_NEUTRAL), 
                        szErrorDesc, sizeof(szErrorDesc), NULL);
                }
           ...
           {
                szErrorName[0] = '\0';
                StringCchCopyA(szErrorDesc,sizeof(szErrorDesc),"This is a FACILITY_CONTROL error that is specific to the control's interface that returned this error. See the documentation of the interface that returned this error for information about this HRESULT.");
           }
           else  if (HRESULT_FACILITY(hr) == FACILITY_ITF && HRESULT_CODE(hr) >= 0x200)
           {
                szErrorName[0] = '\0';
                StringCchCopyA(szErrorDesc,sizeof(szErrorDesc),"This is a FACILITY_ITF error that is specific to the interface that returned this error. See the documentation of the interface that returned this error for information about this HRESULT.");
           }
           else 
           {
              StringCchCopyA(szErrorName,sizeof(szErrorName),"Unknown or new error");  // Not FACILITY_WIN32 or FACILITY_ITF(code >= 0x200) or FACILTY_CONTROL(code >= 600)
              FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM, NULL, hr, 
                             MAKELANGID(LANG_NEUTRAL, SUBLANG_NEUTRAL), 
                             szErrorDesc, sizeof(szErrorDesc), NULL);
           }
	...
	{
		for(char *p=szErrorDesc;*p;p++)
		{
			if(*p<' ') *p=' ';
		}
	}
   
    StringCchPrintfA(szBuf,sizeof(szBuf),"[HR=%lx/FACILITY = %s/%s : %s]", hr,szFacility,szErrorName,szErrorDesc);

	toAppend.assign( szBuf );
}
